...
def update_bonds(receptors, bonds, x1, x2, x3, rmat, dt, k0_on, k0_off,
    ...
    if lam == 0:
        ...
        pw_lengths = np.linalg.norm(true_receptors[:, None, :]
                                    - ligands[None, :, :], axis=-1)
        pw_rates = k0_on * np.exp(-eta_ts * (pw_lengths - lam) ** 2)
...
def time_step(dt, x1, x2, x3, r_matrix, forces, torques, exact_vels, n_nodes=8,
              ...
              precompute_array=None, level=0):
    valid_test_nodes = 48

    if level >= 8:
        raise OverflowError('max recursions reached in time_step')
    ...
    if receptors is not None:
        ...
        f0 = 1.25e9
        tau = 2000.
        rep_force = (f0 * (tau * np.exp(-tau * center[0]))
                     / (1 - np.exp(-tau * center[0])))
    ...
    if order == '1st':
        ...
        try:
            ...
            # I still need to fix what I'm doing with forces
            if save_quad_matrix_info:
            ...
            half_step2 = time_step(
                ...
                exact_vels, n_nodes, a, b, domain, order, proc,
                save_quad_matrix_info, receptors, bonds, eta, eta_ts, kappa,
                lam, k0_on, k0_off, precompute_array=precompute_array,
                level=level+1)
