...
def time_step(dt, x1, x2, x3, r_matrix, forces, torques, exact_vels, n_nodes=8,
    ...
    if receptors is not None:
        ...
        forces, torques = find_bond_forces(receptors, bonds, center, r_matrix, kappa, lam)
        # Try a repulsive force
        sep, point = find_min_separation(center[0], r_matrix[:, 0], loc=True)
        f0 = 1.25e9
        tau = 2000.
    ...
    if order == '1st':
        ...
        if (not valid_orientation(valid_test_nodes, new_rmat[:, 0],
            ...
            if receptors is not None:
                new_bonds = update_bonds(receptors, bonds, x1, x2, x3,
                                         r_matrix, dt, k0_on, k0_off, eta,
                                         eta_ts, lam)
            else:
                new_bonds = None
            # dt_list = [dt]
