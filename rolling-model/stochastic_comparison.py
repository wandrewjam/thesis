...
def stochastic_comparison(num_iterations=100, num_samples=200, M=100, N=100, time_steps=1000, bond_max=100,
    ...
    np.savez('./data/alg1_N{:d}_bmax{:d}_eta{:g}_gamma{:g}'.format(N, bond_max, eta_om, gamma),
             t1, v1_array, om1_array, t1=t1, v1=v1_array, om1=om1_array, n1=n1_array)
    np.savez('./data/alg2_N{:d}_bmax{:d}_eta{:g}_gamma{:g}'.format(N, bond_max, eta_om, gamma),
             t2, v2_array, om2_array, t2=t2, v2=v2_array, om2=om2_array, n2=n2_array)
    ...
    for i in range(num_iterations):
        ...
        print('Completed {:d} of {:d} variable time-step experiments. This run took {:g} seconds.'.format(
            i+1, num_iterations, end - start))

    np.savez('./data/v_ssa_N{:d}_bmax{:d}_eta{:g}_gamma{:g}'.format(N, bond_max, eta_om, gamma), *v3_list)
    np.savez('./data/om_ssa_N{:d}_bmax{:d}_eta{:g}_gamma{:g}'.format(N, bond_max, eta_om, gamma), *om3_list)
    np.savez('./data/t_ssa_N{:d}_bmax{:d}_eta{:g}_gamma{:g}'.format(N, bond_max, eta_om, gamma), *t3_list)
    np.savez('./data/n_ssa_N{:d}_bmax{:d}_eta{:g}_gamma{:g}'.format(N, bond_max, eta_om, gamma), *n3_list)
