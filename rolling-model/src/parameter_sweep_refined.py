...
def parameter_sweep(kap_vec, eta, delta, proc=4):
    ...
    result = [
        ...
        for kappa in kap_vec]
    result = [res.get() for res in result]

    M = [res[0] for res in result]
    T = [res[1] for res in result]

    M = np.reshape(M, newshape=(kap_vec.shape + omegas.shape))
    T = np.reshape(T, newshape=(kap_vec.shape + omegas.shape))
...
if __name__ == '__main__':
    ...
    np.savez_compressed('../data/toy-model/parameter_sweep_refined.npz', M, T,
                        kap_vec, eta, delta, omegas, M=M, T=T,
                        kap_vec=kap_vec, eta=eta, delta=delta,
                        omegas=omegas)
