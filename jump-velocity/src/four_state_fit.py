...
if __name__ == '__main__':
    ...
    for expt, trajectories in experiments.items():
        ...
        for trajectory in trajectories:
            y_save, t_save = process_trajectory(
                trajectory, absolute_pause_threshold=0.5)[1:]
        ...
        print('Fast binding parameters')
        print('k_off = {}, k_on = {}, mean velocity (inc. pauses): {}'.format(
            k_off, k_on, np.mean(v_combined)))
        print('mean dwells per trajectory: {}'.format(
            np.mean(ndwell_combined)))
        print('number of trajectories: {}'.format(len(ndwell_combined)))
        try:
            print('min trajectory velocity (inc. pauses): {}'.format(
                np.amin(v_combined)))
        except ValueError:
            print('No intermediate velocity data to analyze')
    ...
    for expt, trajectories in experiments.items():
        ...
        start = timer()
        full_objective(initial_guess, v_combined, vmin, N_obj)
        end = timer()
