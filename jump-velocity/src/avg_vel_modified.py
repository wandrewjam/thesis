...
from jv_modified import read_parameter_file
...
def main(filename, show_plot=False):
    ...
    u0_bdy, u1_bdy, T_bdy = data['u0_bdy'], data['u1_bdy'], data['T_bdy']
    pars = read_parameter_file(filename)

    s_mask = s_eval > 4./5
    F = cumtrapz((u1_bdy + T_bdy)/(1 - np.exp(-pars['beta'])), s_eval,
                 initial=0)
    F_all = cumtrapz(u0_bdy + u1_bdy + T_bdy, s_eval, initial=0)
    fig_av, ax_av = plt.subplots()
    ax_av.plot(1 / s_eval[s_mask], s_eval[s_mask] ** 2
               * (u1_bdy[s_mask] + T_bdy[s_mask]),
               label='Velocity PDF of filtered plts')
    ax_av.plot(1 / s_eval[s_mask], 1 - F_all[s_mask],
               label='Velocity CDF of all plts')
    ...
    F_all_dat = np.interp(v_dat, (1 / s_eval[s_mask])[::-1], (1 - F_all[s_mask])[::-1])
    np.savetxt(dat_dir + filename + '-dst.dat',
               np.stack((v_dat, f_dat, F_dat, F_all_dat), axis=-1))
