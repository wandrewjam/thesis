...
\pgfplotstableset{
  search path = {/Users/andrewwork/thesis/reg_stokeslets/3d/},
  col sep = comma,
  every head row/.style = {before row = {
      \toprule
      & & \multicolumn{4}{c}{$F^*$ from Regularized Stokeslets} & \\
      \cmidrule{3-6}
    },
    after row = \midrule,
  },
...
\item For reference, here are the discretization sizes for each $S$:
  ...
  \begin{table}[h]
    \centering
    \pgfplotstabletypeset{par_table.dat}
  ...
  \begin{table}[h]
    \centering
    \pgfplotstabletypeset{prp_table.dat}
