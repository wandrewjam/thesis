...
Our goal in this chapter is to develop a model that simplifies the
...
We found $f(v; a, \epsilon_1)$ above in terms of the solution of the
...
An iterative algorithm needs an initial guess, which can be chosen as
...
\begin{figure}
  ...
  \caption[Model fits]{Results of the ML estimates for the reduced
    ...
    data, the maximum likelihood parameters are $a = 0.2182$ and
    $\epsilon_1 = 0.1185$ for the reduced model, and $a = 0.1995$ and
    $\epsilon_1 = 0.1006$ for the full model. The true parameters are
    $a = 0.2$ and $\epsilon_1 = 0.1$. Total number of simulated data
    points: $N = 1024$}
  ...
  \begin{subfigure}{0.54\textwidth}
    \begin{tikzpicture}[baseline]
      \begin{axis}[
        ...
        \draw[thin, dashed] ({axis cs:.2, 0}|-{rel axis cs:0, 0})
        -- ({axis cs:.2, 0}|-{rel axis cs:0, 1});
        \addplot+ [boxplot] table[y index=0]
        {bootstrap/small-a-boot.dat};
...
These equalities can be simultaneously solved to get the following
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{col_prp_traj_sim}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{col_whl_traj_sim}
  \caption{Experimental and simulated platelet trajectories on
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{col_vel_sim}
...
\ITA{IIb}\ITB{3}-fibrinogen increases with integrin activation, while
the unbinding rate remains unchanged, is consistent with the
observations of \cite{Litvinov2012}.

Finally, we estimated the capture rates of platelets in each of the
...
% For Vlado, I should look into which of their experiments I am using,
% and what the hct is for the whole blood

% specify what the features are captured?




% Stuff to think about:
% - Describe what this section adds to the thesis
% - Contextualize the results in terms of the available data, and
% published research
% - Talk (maybe just a bit?) about future directions, and what can be
